...
const apiKey = "3de43bad810fca47eb18b1735106b1b2";
  ...
  cityNames.forEach((city) => {
    ...
    fetch(url)
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Error: ${response.status} - ${response.statusText} for ${city}`
          );
        }
...
fetchWeatherBtn.addEventListener("click", () => {
  const cities = cityInput.value
    .split(",")
    .map((city) => city.trim())
    .filter((city) => city && !cityNames.includes(city)); // New cities only

  if (cities.length === 0) {
    weatherCards.innerHTML =
      "<p>Please enter a valid city name or avoid duplicates.</p>";
    return;
  }
